...
import { Plus, ChevronDown, ChevronLeft, ArrowLeftRight, ArrowRight, Banknote, Check } from 'lucide-react'
import db, { getOrCreateBalanceAdjustmentCategory, type Account, type AccountType } from '../db'
  ...
  return (
    ...
    >
      ...
      {(accounts ?? []).map((account) => {
        const balance = accountBalance(account, transactions ?? [], transfers ?? [], categoriesById)
                  ...
                  {accountHistory.length === 0 ? (
                    ...
                    <ul className="max-h-64 divide-y divide-slate-100 overflow-y-auto dark:divide-slate-800">
                      {accountHistory.slice(0, 20).map((h) => (
                        <li key={h.id} className="flex justify-between gap-2 py-2.5 text-sm">
